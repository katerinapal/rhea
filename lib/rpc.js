...
var Client = function Client(container, address) {
    var u = _url2.default.parse(address);
...
var Cache = function Cache(ttl, purged) {
    this.ttl = ttl;
    this.purged = purged;
    this.entries = {};
...
var LinkCache = function LinkCache(factory, ttl) {
    this.factory = factory;
    this.cache = new Cache(ttl, function (link) {
        link.close();
    });
};
...
var Server = function Server(container, address, options) {
...
Server.prototype._connection_open = function () {
    if (match('ANONYMOUS-RELAY', this.connection.remote.open.offered_capabilities)) {
        ...
        this._send = function (msg) {
            var s = cache.get(msg.to);if (s) s.send(msg);
        };
...
Server.prototype.bind_sync = function (f, name) {
    this.callbacks[name || f.name] = function (args, callback) {
        var result = f(args);callback(result);
    };
};
...
exports.rpcjs = mod_rpcjs = {
    server: function server(container, address, options) {
        return new Server(container, address, options);
    },
    client: function client(connection, address) {
        return new Client(connection, address);
    }
};
var mod_rpcjs;
exports.rpcjs = mod_rpcjs;