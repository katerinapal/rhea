...
function get_connect_fn(options) {
    if (options.transport === undefined || options.transport === 'tcp') {
        return _net2.default.connect;
    } else if (options.transport === 'tls' || options.transport === 'ssl') {
        return _tls2.default.connect;
    } else {
...
var Connection = function Connection(options, container) {
    this.options = {};
    if (options) {
    ...
    if (!this.options.container_id) {
        this.options.container_id = container ? container.id : (0, _util.generate_uuid)();
    }
    ...
    this.local = {};
    this.remote = {};
    this.local.open = _frames.frames.open(this.options);
    this.local.close = _frames.frames.close({});
    this.session_policy = session_per_connection(this);
    this.amqp_transport = new _transport.Transport(this.options.id, AMQP_PROTOCOL_ID, _frames.frames.TYPE_AMQP, this);
    ...
    Object.defineProperty(this, 'error', { get: function get() {
...
Connection.prototype.dispatch = function (name) {
    _log.logjs.events('Connection got event: %s', name);
...
Connection.prototype.reconnect = function () {
    this.scheduled_reconnect = undefined;
    _log.logjs.reconnect('reconnecting...');
    this.reset();
    this._connect(this.options.connection_details(this.conn_established_counter));
...
Connection.prototype.accept = function (socket) {
    this.is_server = true;
    _log.logjs.io('[%s] client accepted: %s', this.id, get_socket_id(socket));
...
Connection.prototype.init = function (socket) {
    ...
    if (this.is_server) {
        ...
        if (this.socket.encrypted && this.socket.authorized && this.get_option('enable_sasl_external', false)) {
            mechs = _sasl.sasljs.server_add_external(mechs ? (0, _util.clone)(mechs) : {});
        }
        if (mechs) {
            if (mechs.ANONYMOUS !== undefined && !this.get_option('require_sasl', false)) {
                this.sasl_transport = new _sasl.sasljs.Selective(this, mechs);
            } else {
                this.sasl_transport = new _sasl.sasljs.Server(this, mechs);
            }
        } else {
            if (!this.get_option('disable_sasl', false)) {
                var anon = _sasl.sasljs.server_mechanisms();
                anon.enable_anonymous();
                this.sasl_transport = new _sasl.sasljs.Selective(this, anon);
            }
        }
    } else {
        ...
        if (!mechanisms) {
            ...
            if (username) {
                mechanisms = _sasl.sasljs.client_mechanisms();
                if (password) mechanisms.enable_plain(username, password);else mechanisms.enable_anonymous(username);
            }
        }
        if (this.socket.encrypted && this.options.cert && this.get_option('enable_sasl_external', false)) {
            if (!mechanisms) mechanisms = _sasl.sasljs.client_mechanisms();
            mechanisms.enable_external();
        }

        if (mechanisms) {
            this.sasl_transport = new _sasl.sasljs.Client(this, mechanisms, this.options.sasl_init_hostname || this.options.servername || this.options.host);
        }
    }
...
Connection.prototype.connected = function () {
    ...
    _log.logjs.io('[%s] connected %s', this.options.id, get_socket_id(this.socket));
    this.output();
};
...
Connection.prototype._handle_error = function () {
    ...
    if (error) {
        ...
        if (!this._is_fatal(error.condition)) {
            ...
            this.dispatch('error', new _errors.errorsjs.ConnectionError(error.description, error.condition, this));
        }
        return true;
    } else {
        return false;
...
Connection.prototype.input = function (buff) {
    ...
    try {
        ...
        _log.logjs.io('[%s] read %d bytes', this.options.id, buff.length);
...
Connection.prototype._disconnected = function () {
    ...
    if (!this.is_closed() && this.scheduled_reconnect === undefined) {
        ...
        if (!this.is_server && !this.transport_error && this.options.reconnect) {
            ...
            if (delay >= 0) {
                _log.logjs.reconnect('Scheduled reconnect in ' + delay + 'ms');
...
Connection.prototype.create_session = function () {
    ...
    while (this.local_channel_map[i]) {
        i++;
    }var session = new _session.Session(this, i);
...
Connection.prototype.each_receiver = function (action, filter) {
    this.each_link(action, (0, _util.receiver_filter)(filter));
};

Connection.prototype.each_sender = function (action, filter) {
    this.each_link(action, (0, _util.sender_filter)(filter));
};
...
Connection.prototype.on_open = function (frame) {
    if (this.state.remote_opened()) {
        ...
        throw new _errors.errorsjs.ProtocolError('Open already received');
    }
};
...
Connection.prototype.on_close = function (frame) {
    if (this.state.remote_closed()) {
        ...
        throw new _errors.errorsjs.ProtocolError('Close already received');
    }
};
...
Connection.prototype._write_frame = function (channel, frame, payload) {
    this.amqp_transport.encode(_frames.frames.amqp_frame(channel, frame, payload));
    this.output();
};
...
Connection.prototype.on_begin = function (frame) {
    ...
    if (frame.performative.remote_channel === null || frame.performative.remote_channel === undefined) {
        ...
        if (!session) throw new _errors.errorsjs.ProtocolError('Invalid value for remote channel ' + frame.performative.remote_channel);
    }
...
function delegate_to_session(name) {
    Connection.prototype['on_' + name] = function (frame) {
        ...
        if (!session) {
            throw new _errors.errorsjs.ProtocolError(name + ' received on invalid channel ' + frame.channel);
        }
...
exports.Connection = mod_Connection = Connection;
exports.Connection = mod_Connection;