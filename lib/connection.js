...
import { Session as session_Session } from "./session.js";
...
function get_connect_fn(options) {
    if (options.transport === undefined || options.transport === 'tcp') {
        return ext_net_net.connect;
    } else if (options.transport === 'tls' || options.transport === 'ssl') {
        return ext_tls_tls.connect;
    } else {
...
var Connection = function (options, container) {
    ...
    if (!this.options.container_id) {
        this.options.container_id = container ? container.id : utiljs_generate_uuid();
    }
    ...
    this.local = {};
    this.remote = {};
    this.local.open = frames_frames.open(this.options);
    this.local.close = frames_frames.close({});
    this.session_policy = session_per_connection(this);
    this.amqp_transport = new transport_Transport(this.options.id, AMQP_PROTOCOL_ID, frames_frames.TYPE_AMQP, this);
...
Connection.prototype.dispatch = function(name) {
    log_logjs.events('Connection got event: %s', name);
...
Connection.prototype.reconnect = function () {
    this.scheduled_reconnect = undefined;
    log_logjs.reconnect('reconnecting...');
    this.reset();
    this._connect(this.options.connection_details(this.conn_established_counter));
...
Connection.prototype.accept = function (socket) {
    this.is_server = true;
    log_logjs.io('[%s] client accepted: %s', this.id, get_socket_id(socket));
...
Connection.prototype.init = function (socket) {
    ...
    if (this.is_server) {
        ...
        if (this.socket.encrypted && this.socket.authorized && this.get_option('enable_sasl_external', false)) {
            mechs = sasl.server_add_external(mechs ? utiljs_clone(mechs) : {});
        }
        if (mechs) {
...
Connection.prototype.connected = function () {
    ...
    log_logjs.io('[%s] connected %s', this.options.id, get_socket_id(this.socket));
    this.output();
};
...
Connection.prototype.input = function (buff) {
    ...
    try {
        ...
        log_logjs.io('[%s] read %d bytes', this.options.id, buff.length);
...
Connection.prototype._disconnected = function () {
    ...
    if (!this.is_closed() && this.scheduled_reconnect === undefined) {
        ...
        if (!this.is_server && !this.transport_error && this.options.reconnect) {
            ...
            if (delay >= 0) {
                log_logjs.reconnect('Scheduled reconnect in ' + delay + 'ms');
...
Connection.prototype.create_session = function () {
    ...
    var session = new session_Session(this, i);
...
Connection.prototype.each_receiver = function (action, filter) {
    this.each_link(action, utiljs_receiver_filter(filter));
};

Connection.prototype.each_sender = function (action, filter) {
    this.each_link(action, utiljs_sender_filter(filter));
};
...
Connection.prototype._write_frame = function (channel, frame, payload) {
    this.amqp_transport.encode(frames_frames.amqp_frame(channel, frame, payload));
    this.output();
};
...
export { mod_Connection as Connection };
