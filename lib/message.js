import { logjs as log } from "./log.js";
...
function define_composite_section(def) {
    var c = types_types.define_composite(def);
...
function define_map_section(def) {
    ...
    var unwrap = function (msg, section) {
        msg[def.name] = types_types.unwrap_map_simple(section);
    };
    var wrap = function (sections, msg) {
        if (msg[def.name]) {
            sections.push(types_types.described_nc(types_types.wrap_ulong(descriptor.numeric), types_types.wrap_map(msg[def.name])));
        }
    };
...
Section.prototype.described = function () {
    return types_types.described(types_types.wrap_ulong(this.typecode), types_types.wrap(this.content));
};
...
define_section({numeric:0x75, symbolic:'amqp:data:binary'}, function (msg, section) { msg.body = new Section(0x75, types_types.unwrap(section)); });
define_section({numeric:0x76, symbolic:'amqp:amqp-sequence:list'}, function (msg, section) { msg.body = new Section(0x76, types_types.unwrap(section)); });
define_section({numeric:0x77, symbolic:'amqp:value:*'}, function (msg, section) { msg.body = types_types.unwrap(section); });
...
function wrap_body (sections, msg) {
    if (msg.body && msg.body.constructor === Section) {
        ...
        sections.push(types_types.described(types_types.wrap_ulong(0x77), types_types.wrap(msg.body)));
    }
}
...
message.encode = function(msg) {
    ...
    var writer = new types_types.Writer();
...
message.decode = function(buffer) {
    var msg = new Message();
    var reader = new types_types.Reader(buffer);
