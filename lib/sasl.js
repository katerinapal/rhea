...
function extract(buffer) {
    ...
    while (i < buffer.length) {
        if (buffer[i] === 0x00) {
            if (i > start) results.push(buffer.toString('utf8', start, i));else results.push(null);
            start = ++i;
        } else {
            ++i;
        }
...
var PlainServer = function PlainServer(callback) {
...
var PlainClient = function PlainClient(username, password) {
...
var AnonymousServer = function AnonymousServer() {
    this.outcome = undefined;
    this.username = undefined;
};
...
var AnonymousClient = function AnonymousClient(name) {
...
var ExternalServer = function ExternalServer() {
    this.outcome = undefined;
    this.username = undefined;
};
...
var ExternalClient = function ExternalClient() {
    this.username = undefined;
};
 ...
 */
var SaslServer = function SaslServer(connection, mechanisms) {
    this.connection = connection;
    this.transport = new _transport.Transport(connection.amqp_transport.identifier, SASL_PROTOCOL_ID, _frames.frames.TYPE_SASL, this);
    ...
    this.transport.encode(_frames.frames.sasl_frame(_frames.frames.sasl_mechanisms({ sasl_server_mechanisms: mechlist }).described()));
};

SaslServer.prototype.do_step = function (challenge) {
    if (this.mechanism.outcome === undefined) {
        this.transport.encode(_frames.frames.sasl_frame(_frames.frames.sasl_challenge({ 'challenge': challenge }).described()));
    } else {
        this.outcome = this.mechanism.outcome ? sasl_codes.OK : sasl_codes.AUTH;
        this.transport.encode(_frames.frames.sasl_frame(_frames.frames.sasl_outcome({ code: this.outcome }).described()));
...
var SaslClient = function SaslClient(connection, mechanisms, hostname) {
    this.connection = connection;
    this.transport = new _transport.Transport(connection.amqp_transport.identifier, SASL_PROTOCOL_ID, _frames.frames.TYPE_SASL, this);
    this.next = connection.amqp_transport;
    this.mechanisms = mechanisms;
...
SaslClient.prototype.on_sasl_mechanisms = function (frame) {
    ...
    if (this.mechanism) {
        ...
        this.transport.encode(_frames.frames.sasl_frame(_frames.frames.sasl_init(init).described()));
    } else {
        this.failed = true;
...
SaslClient.prototype.on_sasl_challenge = function (frame) {
    ...
    this.transport.encode(_frames.frames.sasl_frame(_frames.frames.sasl_response({ 'response': response }).described()));
};
...
var SelectiveServer = function SelectiveServer(connection, mechanisms) {
    ...
    if (!this.header_received) {
        if (buffer.length < 8) {
            ...
            if (this.selected === undefined) {
                throw new _errors.errorsjs.ProtocolError('Invalid AMQP protocol id ' + this.header_received.protocol_id);
            }
        }
    }
...
var default_server_mechanisms = {
    enable_anonymous: function enable_anonymous() {
        this['ANONYMOUS'] = function () {
            return new AnonymousServer();
        };
    },
    enable_plain: function enable_plain(callback) {
        this['PLAIN'] = function () {
...
var default_client_mechanisms = {
    enable_anonymous: function enable_anonymous(name) {
        this['ANONYMOUS'] = function () {
            return new AnonymousClient(name);
        };
    },
    enable_plain: function enable_plain(username, password) {
        this['PLAIN'] = function () {
            return new PlainClient(username, password);
        };
    },
    enable_external: function enable_external() {
...
exports.sasljs = mod_sasljs = {
    Client: SaslClient,
    Server: SaslServer,
    Selective: SelectiveServer,
    server_mechanisms: function server_mechanisms() {
        return Object.create(default_server_mechanisms);
    },
    client_mechanisms: function client_mechanisms() {
        return Object.create(default_client_mechanisms);
    },
    server_add_external: function server_add_external(mechs) {
