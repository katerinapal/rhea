...
function write_dispositions(deliveries) {
    ...
    for (i = 0; i < deliveries.length; i++) {
        ...
        if (!_message.message.are_outcomes_equivalent(last.state, delivery.state) || last.settled !== delivery.settled || next_id !== delivery.id) {
            first.link.session.output(_frames.frames.disposition({ 'role': first.link.is_receiver(), 'first': first.id, 'last': last.id, 'state': first.state, 'settled': first.settled }).described());
            first = delivery;
            last = delivery;
            next_id = delivery.id;
        } else {
...
var Outgoing = function Outgoing(connection) {
    ...
    this.window = _types.types.MAX_UINT;
    ...
    for (var i = fields.first; i <= last; i++) {
        ...
        if (d && !d.remote_settled) {
            ...
            if (fields.state && fields.state !== d.remote_state) {
                d.remote_state = _message.message.unwrap_outcome(fields.state);
                updated = true;
            }
            if (updated) {
                this.updated.push(d);
            ...
            if (d.link.has_credit()) {
                if (this.transfer_window() >= d.data.length) {
                    ...
                    for (var i = 0; i < d.data.length; i++) {
                        ...
                        var transfer = _frames.frames.transfer({ 'handle': d.link.local.handle, 'message_format': d.format, 'delivery_id': d.id, 'delivery_tag': d.tag, 'settled': d.settled, 'more': more });
                    ...
                    _log.logjs.flow('Incoming window of peer preventing sending further transfers: remote_window=%d, remote_next_transfer_id=%d, next_transfer_id=%d', this.remote_window, this.remote_next_transfer_id, this.next_transfer_id);
                    break;
                }
            } else {
                _log.logjs.flow('Link has no credit');
                break;
            }
        } else {
    ...
    if (receiver.is_open()) {
        ...
        if (last && last.incomplete) {
            ...
            current.accept = function () {
                this.update(undefined, _message.message.accepted().described());
            };
            current.release = function (params) {
                if (params) {
                    this.update(undefined, _message.message.modified(params).described());
                } else {
                    this.update(undefined, _message.message.released().described());
                }
            };
            current.reject = function (error) {
                this.update(true, _message.message.rejected({ 'error': error }).described());
            };
            current.modified = function (params) {
                this.update(true, _message.message.modified(params).described());
            };
        ...
        if (current.incomplete) {
            ...
            receiver.dispatch('message', receiver._context({ 'message': _message.message.decode(data), 'delivery': current }));
        }
    }
};
...
var Session = function Session(connection, local_channel) {
    this.connection = connection;
    this.outgoing = new Outgoing(connection);
    this.incoming = new Incoming();
    this.state = new _endpoint.EndpointState();
    this.local = { 'channel': local_channel, 'handles': {} };
    this.local.begin = _frames.frames.begin({ next_outgoing_id: this.outgoing.next_transfer_id, incoming_window: this.incoming.window, outgoing_window: this.outgoing.window });
    this.local.end = _frames.frames.end();
    this.remote = { 'handles': {} };
...
Session.prototype.dispatch = function (name) {
    _log.logjs.events('Session got event: %s', name);
...
Session.prototype.create_sender = function (name, opts) {
    return this.create_link(name, _link.linkjs.Sender, opts);
};

Session.prototype.create_receiver = function (name, opts) {
    return this.create_link(name, _link.linkjs.Receiver, opts);
};
...
function attach(factory, args, remote_terminus) {
    ...
    if (!opts.name) opts.name = (0, _util.generate_uuid)();
...
Session.prototype.get_option = function (name, default_value) {
    if (this.options[name] !== undefined) return this.options[name];else return this.connection.get_option(name, default_value);
};
...
Session.prototype.find_sender = function (filter) {
    return this.find_link((0, _util.sender_filter)(filter));
};

Session.prototype.find_receiver = function (filter) {
    return this.find_link((0, _util.receiver_filter)(filter));
};
...
Session.prototype.each_receiver = function (action, filter) {
    this.each_link((0, _util.receiver_filter)(filter));
};

Session.prototype.each_sender = function (action, filter) {
    this.each_link((0, _util.sender_filter)(filter));
};
...
Session.prototype._write_flow = function (link) {
    ...
    this.output(_frames.frames.flow(fields).described());
};
...
Session.prototype.on_disposition = function (frame) {
    if (frame.performative.role) {
        _log.logjs.events('Received disposition for outgoing transfers');
        this.outgoing.on_disposition(frame.performative);
    } else {
        _log.logjs.events('Received disposition for incoming transfers');
...
exports.Session = mod_Session = Session;
exports.Session = mod_Session;