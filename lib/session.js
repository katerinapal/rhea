...
import { linkjs as link } from "./link.js";
import { logjs as log_logjs } from "./log.js";
import { message as message_message } from "./message.js";
import { types as types_types } from "./types.js";

import {
    generate_uuid as utiljs_generate_uuid,
    sender_filter as utiljs_sender_filter,
    receiver_filter as utiljs_receiver_filter,
} from "./util.js";

import { EndpointState as endpoint_EndpointState } from "./endpoint.js";
...
function write_dispositions(deliveries) {
    ...
    for (i = 0; i < deliveries.length; i++) {
        ...
        if (!message_message.are_outcomes_equivalent(last.state, delivery.state) || last.settled !== delivery.settled || next_id !== delivery.id) {
            first.link.session.output(frames_frames.disposition({'role' : first.link.is_receiver(), 'first' : first.id, 'last' : last.id, 'state' : first.state, 'settled' : first.settled}).described());
            first = delivery;
            last = delivery;
            next_id = delivery.id;
        } else {
...
var Outgoing = function (connection) {
    ...
    this.window = types_types.MAX_UINT;
    ...
    for (var i = fields.first; i <= last; i++) {
        ...
        if (d && !d.remote_settled) {
            ...
            if (fields.state && fields.state !== d.remote_state) {
                d.remote_state = message_message.unwrap_outcome(fields.state);
                updated = true;
            }
            if (updated) {
                this.updated.push(d);
            ...
            if (d.link.has_credit()) {
                if (this.transfer_window() >= d.data.length) {
                    ...
                    for (var i = 0; i < d.data.length; i++) {
                        ...
                        var transfer = frames_frames.transfer({'handle':d.link.local.handle,'message_format':d.format,'delivery_id':d.id, 'delivery_tag':d.tag, 'settled':d.settled, 'more':more});
                    ...
                    log_logjs.flow('Incoming window of peer preventing sending further transfers: remote_window=%d, remote_next_transfer_id=%d, next_transfer_id=%d',
                             this.remote_window, this.remote_next_transfer_id, this.next_transfer_id);
                    break;
                }
            } else {
                log_logjs.flow('Link has no credit');
                break;
            }
        } else {
...
Incoming.prototype.on_transfer = function(frame, receiver) {
    ...
    if (receiver.is_open()) {
        ...
        if (last && last.incomplete) {
            ...
            current.accept = function () { this.update(undefined, message_message.accepted().described()); };
            current.release = function (params) {
                if (params) {
                    this.update(undefined, message_message.modified(params).described());
                } else {
                    this.update(undefined, message_message.released().described());
                }
            };
            current.reject = function (error) { this.update(true, message_message.rejected({'error':error}).described()); };
            current.modified = function (params) { this.update(true, message_message.modified(params).described()); };
        ...
        if (current.incomplete) {
            ...
            receiver.dispatch('message', receiver._context({'message':message_message.decode(data), 'delivery':current}));
        }
    }
};
...
var Session = function (connection, local_channel) {
    ...
    this.state = new endpoint_EndpointState();
    this.local = {'channel': local_channel, 'handles':{}};
    this.local.begin = frames_frames.begin({next_outgoing_id:this.outgoing.next_transfer_id,incoming_window:this.incoming.window,outgoing_window:this.outgoing.window});
    this.local.end = frames_frames.end();
    this.remote = {'handles':{}};
...
Session.prototype.dispatch = function(name) {
    log_logjs.events('Session got event: %s', name);
...
function attach(factory, args, remote_terminus) {
    ...
    if (!opts.name) opts.name = utiljs_generate_uuid();
...
Session.prototype.find_sender = function (filter) {
    return this.find_link(utiljs_sender_filter(filter));
};

Session.prototype.find_receiver = function (filter) {
    return this.find_link(utiljs_receiver_filter(filter));
};
...
Session.prototype.each_receiver = function (action, filter) {
    this.each_link(utiljs_receiver_filter(filter));
};

Session.prototype.each_sender = function (action, filter) {
    this.each_link(utiljs_sender_filter(filter));
};
...
Session.prototype._write_flow = function (link) {
    ...
    this.output(frames_frames.flow(fields).described());
};
...
Session.prototype.on_disposition = function (frame) {
    if (frame.performative.role) {
        log_logjs.events('Received disposition for outgoing transfers');
        this.outgoing.on_disposition(frame.performative);
    } else {
        log_logjs.events('Received disposition for incoming transfers');
