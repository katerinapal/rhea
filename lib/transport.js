...
Transport.prototype.encode = function (frame) {
    var buffer = frames_frames.write_frame(frame);
...
Transport.prototype.write = function (socket) {
    if (!this.header_sent) {
        ...
        frames_frames.write_header(buffer, header);
        socket.write(buffer);
...
Transport.prototype.read = function (buffer) {
    ...
    while (offset < (buffer.length - 4) && !this.read_complete) {
        ...
        if (buffer.length < offset + frame_size) {
            ...
            var frame = frames_frames.read_frame(buffer.slice(offset, offset + frame_size));
...
export { mod_Transport as Transport };
