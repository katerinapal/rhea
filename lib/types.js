...
types.wrap_ulong = function (l) {
    if (Buffer.isBuffer(l)) {
        ...
        if (l === 0) return types.Ulong0();else return l > 255 ? types.Ulong(l) : types.SmallUlong(l);
    }
};
types.wrap_uint = function (l) {
    if (l === 0) return types.Uint0();else return l > 255 ? types.Uint(l) : types.SmallUint(l);
};
...
types.wrap_array = function (l, code, descriptors) {
    if (code) {
        ...
        throw new _errors.errorsjs.TypeError('An array must specify a type for its elements');
    }
};
types.wrap = function (o) {
    ...
    if (t === 'string') {
        ...
        if (isNaN(o)) {
            throw new _errors.errorsjs.TypeError('Cannot wrap NaN! ' + o);
...
types.wrap_message_id = function (o) {
    ...
    if (t === 'string') {
        ...
        //TODO handle uuids
        throw new _errors.errorsjs.TypeError('invalid message id:' + o);
    }
};
...
function get_type(code) {
    ...
    if (!type) {
        throw new _errors.errorsjs.TypeError('Unrecognised typecode: ' + hex(code));
    }
    return type;
}
...
types.Reader.prototype.read_uint = function (width) {
    ...
    if (width === 1) {
        ...
        throw new _errors.errorsjs.TypeError('Unexpected width for uint ' + width);
    }
};
...
types.Reader.prototype.read_value = function (type) {
    if (type.width === 0) {
        ...
        throw new _errors.errorsjs.TypeError('Invalid category for type: ' + type);
    }
};
...
types.Writer.prototype.write_fixed_width = function (type, value) {
    ...
    if (type.write) {
        ...
        throw new _errors.errorsjs.TypeError('Cannot handle write for ' + type);
    }
};
...
types.Writer.prototype.write = function (o) {
    if (o.type === undefined) {
        throw new _errors.errorsjs.TypeError('Cannot write ' + JSON.stringify(o));
    }
...
types.Writer.prototype.write_value = function (type, value, constructor /*for arrays only*/) {
    if (type.width === 0) {
        ...
        throw new _errors.errorsjs.TypeError('Invalid category ' + type.category + ' for type: ' + type);
    }
};
...
function get_constructor(typename) {
    ...
    throw new _errors.errorsjs.TypeError('TODO: Array of type ' + typename + ' not yet supported');
}
    ...
    if (instance !== undefined && instance !== null) {
        if (Array.isArray(instance)) {
            if (!definition.multiple) {
                throw new _errors.errorsjs.TypeError('Field ' + definition.name + ' does not support multiple values, got ' + JSON.stringify(instance));
            }
            ...
            if (wrapper) {
                ...
                throw new _errors.errorsjs.TypeError('No wrapper for field ' + definition.name + ' of type ' + definition.type);
            }
        }
    } else if (definition.mandatory) {
        throw new _errors.errorsjs.TypeError('Field ' + definition.name + ' is mandatory');
    } else {
        return types.Null();
    }
}
...
add_type({ name: 'error',
    code: 0x1d,
    fields: [{ name: 'condition', type: 'symbol', mandatory: true }, { name: 'description', type: 'string' }, { name: 'info', type: 'map' }]
});

var mod_types;

exports.types = mod_types = types;
exports.types = mod_types;