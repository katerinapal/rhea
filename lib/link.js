...
var _terminus = require("./terminus.js");
...
var FlowController = function FlowController(window) {
    this.window = window;
};
...
function auto_accept(context) {
    context.delivery.update(undefined, _message.message.accepted().described());
}

function LinkError(message_message, condition, link) {
    Error.call(this);
    Error.captureStackTrace(this, this.constructor);
    this.message = message_message;
    this.condition = condition;
    this.description = message_message;
    this.link = link;
}
_util2.default.inherits(LinkError, Error);

var EventEmitter = _events2.default.EventEmitter;

var link = Object.create(EventEmitter.prototype);
link.dispatch = function (name) {
    _log.logjs.events('Link got event: %s', name);
...
link.set_source = function (fields) {
    this.local.attach.source = _terminus.terminus.source(fields).described();
};
link.set_target = function (fields) {
    this.local.attach.target = _terminus.terminus.target(fields).described();
};
...
link.on_attach = function (frame) {
    if (this.state.remote_opened()) {
        ...
        frame.performative.source = _terminus.terminus.unwrap(frame.performative.source);
        frame.performative.target = _terminus.terminus.unwrap(frame.performative.target);
...
var aliases = ['snd_settle_mode', 'rcv_settle_mode', 'source', 'target', 'max_message_size', 'offered_capabilities', 'desired_capabilities', 'properties'];
...
link.init = function (session, name, local_handle, opts, is_receiver) {
    ...
    this.local.attach = _frames.frames.attach({ 'handle': local_handle, 'name': name, role: is_receiver });
    ...
    this.local.detach = _frames.frames.detach({ 'handle': local_handle, 'closed': true });
...
link.get_option = function (name, default_value) {
    if (this.options[name] !== undefined) return this.options[name];else return this.session.get_option(name, default_value);
};

var Sender = function Sender(session, name, local_handle, opts) {
...
Sender.prototype.send = function (msg, tag) {
    var delivery = this.session.send(this, tag ? tag : this.next_tag(), _message.message.encode(msg), 0);
...
var Receiver = function Receiver(session, name, local_handle, opts) {
...
Receiver.prototype.on_flow = function (frame) {
    ...
    if (frame.performative.drain) {
        if (frame.performative.link_credit > 0) console.error('ERROR: received flow with drain set, but non zero credit');else this.dispatch('receiver_drained', this._context());
    }
};
...
exports.linkjs = mod_linkjs = { 'Sender': Sender, 'Receiver': Receiver };
var mod_linkjs;
exports.linkjs = mod_linkjs;