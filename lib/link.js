...
import { terminus as terminus_terminus } from "./terminus.js";
...
function auto_accept(context) {
    context.delivery.update(undefined, message_message.accepted().described());
}
...
link.set_source = function (fields) {
    this.local.attach.source = terminus_terminus.source(fields).described();
};
link.set_target = function (fields) {
    this.local.attach.target = terminus_terminus.target(fields).described();
};
...
link.on_attach = function (frame) {
    if (this.state.remote_opened()) {
        ...
        frame.performative.source = terminus_terminus.unwrap(frame.performative.source);
        frame.performative.target = terminus_terminus.unwrap(frame.performative.target);
...
link.init = function (session, name, local_handle, opts, is_receiver) {
    ...
    this.local.attach = frames_frames.attach({'handle':local_handle,'name':name, role:is_receiver});
    ...
    this.local.detach = frames_frames.detach({'handle':local_handle, 'closed':true});
...
Sender.prototype.send = function (msg, tag) {
    var delivery = this.session.send(this, tag ? tag : this.next_tag(), message_message.encode(msg), 0);
...
mod_linkjs = {'Sender': Sender, 'Receiver':Receiver};
var mod_linkjs;
export { mod_linkjs as linkjs };
